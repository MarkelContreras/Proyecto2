...
def preparar_embebido():
    ...
    # Preguntar al usuario si quiere usar entrada por argumento o stdin
    modo_input = messagebox.askquestion("Modo de entrada", "¿Quieres usar input por argumento? (No = stdin)")

    if modo_input == "yes":
        servidor_filename = "servidorArgumento.py"
    else:
        servidor_filename = "servidorStdin.py"

    # Ruta del servidor seleccionado
    servidor_path = os.path.join(os.path.dirname(__file__), "Embebido", servidor_filename)
    if not os.path.exists(servidor_path):
        messagebox.showerror("Error", f"No se encontró '{servidor_filename}' en: {servidor_path}")
        return

    # Copiar el servidor seleccionado como servidor.py
    scp_server_selected = ["scp", servidor_path, f"{remote_user}@{remote_ip}:{remote_dir}/servidor.py"]
    print("DEBUG: Enviando", servidor_filename, "como servidor.py con:")
    print(" ".join(scp_server_selected))
    try:
        subprocess.run(scp_server_selected, check=True, capture_output=True, text=True)
        print("Servidor", servidor_filename, "copiado como servidor.py correctamente.")
    except subprocess.CalledProcessError as e:
        messagebox.showerror("Error", f"Error al copiar el servidor:\n{e.stderr}")
        return
